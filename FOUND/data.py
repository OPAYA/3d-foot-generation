...
class FootScanDataset(Dataset):
  ...
  def __init__(self, src, targ_img_size, folder_names: dict):
    ...
    self.camera_params = dict(focal_length=f*fac, principal_point=(cx*fac, cy*fac))
  ...
  def restrict_views(self, n_views: int):
    ...
    w2v_trans = get_world_to_view_transform(R=torch.from_numpy(R), T=torch.from_numpy(T))
    C = w2v_trans.inverse().get_matrix()[:, 3, :3].cpu().detach().numpy() # camera centres
    ...
    if n_views == 1:
      ...
      while len(cam_idxs) > n_views:
        ...
        dists = [(abs(a-b)+abs(c-b)) / 2 for a, b, c in zip(Ys, Ys[1:], Ys[2:])] # middle cameras
      ...
      if os.path.isfile(pth):
        ...
        if targ_size != None:
          ...
          if current_aspect_ratio != targ_aspect_ratio:
            raise ValueError(f"Image {loc} has aspect ratio {current_aspect_ratio}, but target aspect ratio is {targ_aspect_ratio}.")
  ...
  def __getitem__(self, i):
    ...
    skin_masking = True
    if skin_masking:
      mask = self.get_skin_mask(rgb)
      norm_rgb = cv2.bitwise_and(norm_rgb, norm_rgb, mask=mask)
      norm_kappa = cv2.bitwise_and(norm_kappa, norm_kappa, mask=mask)
